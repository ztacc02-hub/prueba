...
async function login(event) {
  ...
  try {
    const loginResponse = await fetch("/api/antel-login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({}) });
...
async function refreshAntelSession() {
  const response = await fetch("/api/antel-login", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
